class OrdersController < ApplicationController
  ...
  def create
    @order = Order.new(units: params[:order][:units].to_i, product: @product, fan: current_user.profile,
                       buyer: @buyer, address: @actual_address)
  ...
  # mercadopago checkout --> delegado al modelo
#  def create_checkout
#    @preferenceData = {
...
#  end
  ...
  def stock_validation_and_response_with(http_response)
    if @product.stock_greater_or_equals_than? @order.units
      ...
      @preference = @order.create_checkout
